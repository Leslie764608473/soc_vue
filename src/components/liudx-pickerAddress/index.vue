	...
	<u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="value" length="auto"
	 ...
	 <view class="u-picker-header" @touchmove.stop.prevent="stop" catchtouchmove="stop">
	 	...
	 	<view class="u-btn-picker u-btn-picker--primary" hover-class="u-opacity"
	 	 :hover-stay-time="150" @touchmove.stop="" @tap.stop="getResult('confirm')">確定</view>
	 </view>
...
<script>
	...
	import {getHometownDict } from '@/api/login';
	export default {
		props: {
			areas: {
				type: Boolean,
		...
		watch: {
			...
			province(val) {
				this.loadCities(this.districtsObj.provinces[this.province].namec);
			},
			city(val) {
				this.loadAreas(this.districtsObj.cities[this.city].namec);
			}
		},
		mounted() {
			this.loadDistrict()
		...
		methods: {
			...
			loadProvinces() { // 加载省份
				this.$http.post(getHometownDict,{"level": 1}).then((res)=>{
					...
					this.loadCities(res.data[0].namec)
				});
			},
			loadCities(namec) {
				this.$http.post(getHometownDict,{"level": 2,"regionNames": [namec]}).then((res)=>{
					this.districtsObj.cities = res.data
					this.loadAreas(res.data[0].namec)
				});
			},
			loadAreas(namec) {
				this.$http.post(getHometownDict,{"level": 3,"regionNames": [namec]}).then((res)=>{
