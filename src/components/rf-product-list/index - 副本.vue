			...
			<block v-for="(item, index) in list" :key="index">
				...
				>
					...
					<view class="rf-pro-content">
						<view class="rf-pro-tit" style="font-size: 28rpx;">{{ item.name }}</view>
						<view v-if="item.brandName">
							<text style="display: inline-block;width: auto;padding: 10rpx 20rpx;background-color: #9bca38;color: white;border-radius: 10rpx;">{{item.brandName}}</text></view>
						<view v-if="item">
							<view class="rf-pro-price">
								<!-- <text class="rf-sale-price" :class="'text-'+themeColor.name">{{ moneySymbol }}{{ item.originalPrice }}</text>
