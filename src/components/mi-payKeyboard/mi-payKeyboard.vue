<template>
	<view class="keyboard-content">
		<view class="content_">
			<view class="title">請輸入密碼</view>
			<view class="password-error">
				<text v-if="passwordError">對不起，您的密碼不正確，請重新輸入</text>
			</view>
			<view class="input-content">
				<view class="input_">
					<view v-for="(item,index) in inputArr" :key="index">
	...
	export default {
		...
		methods: {
			...
			testPassword() {
				...
				// 测试数据  在此处可添加个人的业务代码
				this.$parent.password = passStr;
				this.$parent.useReceiveFn();
				return false;
			},
			// 点击忘记密码 可跳转页面或自定义业务
			forgetPass() {
