				...
				<view class="main">
					...
					<block v-if="tabCurrentIndex === 1">
						<view class="login-type-form">
							...
							<view class="input-item">
								<text class="iconfont iconrili" :class="'text-' + themeColor.name"></text>
								<input
									class="login-type-input"
									type="number"
									name="userYear"
									v-model="loginParams.userYear"
									placeholder="請輸入出生年份,如: 1980"
									maxlength="4"
								/>
							</view>
						</view>
	...
	methods: {
		...
		async toLogin() {
			...
			if(this.tabCurrentIndex == 0) {
				...
				login_param = {
					username: this.loginParams.mobile,
					birYear: this.loginParams.userYear,
					name: this.loginParams.nameZh,
					orgId: this.orgChoseId,
					onlyUuid: uuid
				}
			...
			/* if (login_param.name == "") {
				this.$mHelper.toast("請輸入會員姓名!");
				return;
			} */

			if (this.tabCurrentIndex == 0 && login_param.authCode == "") {
				this.$mHelper.toast("請輸入驗證碼!");
				return;
			}
			if (this.tabCurrentIndex == 1 && login_param.birYear == "") {
				this.$mHelper.toast("請輸入出生年份!");
				return;
			}
			this.btnLoading = true;
			await this.$http
				.post(login,{},{params:login_param})
				.then(res => {
					if(res.code == 200) {
