	...
	<view class="cart">
		<view class="tabr" :style="{ top: headerTop }">
			<view :class="typeClass == 'valid' ? `text-${themeColor.name} on` : ''" @tap="switchType('valid', 0)"
				>自取或郵寄
				<text v-if="state === 0" style="color: #0f7df7;margin-left: 20rpx;">{{ cartList.length }}</text>
			</view>
			<view :class="typeClass == 'used' ? `text-${themeColor.name} on` : ''" @tap="switchType('used', 1)"
				>自取商品
				<text v-if="state === 1" style="color: #0f7df7;margin-left: 20rpx;">{{ cartList.length }}</text>
			</view>
			<view
				:class="typeClass == 'invalid' ? `text-${themeColor.name} on` : ''"
				@tap="switchType('invalid', 2)"
				>郵寄商品
				<text v-if="state === 2" style="color: #0f7df7;margin-left: 20rpx;">{{ cartList.length }}</text>
			</view>
...
export default {
  components: { rfAttrContent },
	...
	methods: {
		...
		selected(index, item) {
			this.selectedList = [];
			this.cartList.forEach((obj,i)=>{
				this.cartList[i].selected = false;
				if(item.cartItem.id == obj.cartItem.id) {
					this.selectedList.push(obj.cartItem.id)
				}
			});
		...
		async createOrder() {
			...
			for (let i = 0; i < len; i++) {
				if (this.cartList[i].selected) {
					objId = this.cartList[i].cartItem.id;
					ids.push(parseInt(this.cartList[i].cartItem.productId, 10));
				}
			}
